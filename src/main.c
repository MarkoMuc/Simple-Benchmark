 ...
 *  - "cpu" -> tests cpu freq., default is 1000 iterations of fib., DONT EVEN
 * USE IT ITS BAD!!!!!!!!
 *
...
int main(int argc, char **argv) {
  ...
  if (argc > 1 && strcmp(argv[1], "disk") == 0) {
    ...
    printf("HELP!\n");
    printf("Every iteration of R/W tests reads/writes 4096Bytes, meaning total "
           "amount of data is bufferSize * maxRuns\n");
    printf(
        "\t- DISK R/W speed test : disk [maxRuns] | default is 1000 = 4.1MB\n");
    printf("\t- RAM R/W speed test: ram [size x] [size y] [maxRuns] | default "
           "is 1000000 = 8.0MB\n");
...
void storage_test(const char path[], long maxRuns) {
  ...
  // BufferSize is the number of elements in the buffer, it is not the size of
  // the buffer in bytes
