...
/* Reads buffSize of Bytes at a time off the disk, for given amount of reads, calculates time and transfer
 * Parameters:
 *     - FILE* read -> file pointer to the file to read
 *     - uint8_t* buffer -> buffer, not really used for anything else than checking for fail
 ...
 *        |-note that it does buffSize * maxRuns of actual reads, since it reads 4096 per every iteration of maxRuns
 */
...
/* Writes 4096 Bytes at onto the disk, for given amount of writes, calculates time and transfer
 * Parameters:
 *     - FILE* write -> file pointer to the file to write to
 *     - uint8_t* buffer -> buffer that holds values to write into the file
 *     - int buffSize -> size of the buffer, writes this amount of bytes per loop
 *     - long maxRuns -> the number of writes that it should perform,
 *        |-note that it does buffSize * maxRuns of actual writes, since it writes 4096 per every iteration of maxRuns
 */
...
/* Reads 1 Bytes at a time from RAM, for given amount of reads, calculates time and transfer
 * Parameters:
...
/* Writes 1 Byte at a time from RAM, for given amount of reads, calculates time and transfer
 * Parameters:
 *     - MemMap* map -> holds the allocated memory and its size in X and Y
 *     - int* indexX -> holds random x indexes generated beforehand
 *     - int* indexY -> holds random y indexes generated beforehand
